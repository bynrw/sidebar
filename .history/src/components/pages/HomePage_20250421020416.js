...
const StatCard = ({ title, value, icon, color, subtitle, percent, trend }) => {
      ...
      <CardContent sx={{ p: 3 }}>
        ...
        {trend && (
          <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
            <Chip 
              size="small" 
              label={`${trend > 0 ? '+' : ''}${trend}% seit letzter Woche`} 
...
const EmergencyCard = ({ title, location, time, type, status, priority }) => {
  ...
  const getStatusChip = (status) => {
    switch(status) {
      case 'aktiv':
        return <Chip size="small" label="Aktiv" color="error" sx={{ height: 20, fontSize: '0.7rem' }} />;
      case 'inbearbeitung':
        return <Chip size="small" label="In Bearbeitung" color="warning" sx={{ height: 20, fontSize: '0.7rem' }} />;
      case 'abgeschlossen':
        return <Chip size="small" label="Abgeschlossen" color="success" sx={{ height: 20, fontSize: '0.7rem' }} />;
      default:
        return <Chip size="small" label="Offen" color="primary" sx={{ height: 20, fontSize: '0.7rem' }} />;
    }
  };
...
const QuickAccessButton = ({ icon, title, path, color }) => {
  const navigate = useNavigate();
  const theme = useTheme();
