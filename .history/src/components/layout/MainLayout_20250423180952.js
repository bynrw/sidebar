...
// Import des Theme Context aus App.js
import { useThemeContext, useFavoritesContext } from '../../App';
import CustomCard from './CustomCard'; // Hinzugefügt: Import der CustomCard für Untermenüs
...
// Hilfsfunktion um passende Icons für Menüpunkte zu erhalten
const getIconForMenuItem = (title) => {
  switch (title) {
    ...
    case 'Info':
      return <InfoIcon />;
    case 'Teilnehmer-Bestnachweis':
      return <FormatListNumberedIcon />;
    case 'Persönliche-Angaben':
      return <PersonIcon />;
    case 'Daten bearbeiten':
      return <EditIcon />;
    case 'Passwort ändern':
      return <SecurityIcon />;
    case 'Benutzerbestand':
      return <GroupsIcon />;
    case 'Liste':
      return <FormatListBulletedIcon />;
    case 'Neuer Benutzer':
      return <AddBoxIcon />;
    case 'Organisationsdaten':
      return <BusinessIcon />;
    case 'Datenbearbeiten':
      return <EditIcon />;
    case 'Deckblatt':
      return <ArticleIcon />;
    case 'Anzeigen':
  ...
  },
  {
    ...
    subMenu: [
      ...
      },
      {
        ...
        path: '/administration/systemeinstellungen'
      },
      {
        title: 'Protokolle',
      ...
      },
      {
        ...
        path: '/administration/datensicherung'
      }
    ]
  },
  {
...
const manvFaelle = [
  ...
  {
    id: 3,
    titel: "MANV Krefeld - Chemieunfall in Fabrik",
    patienten: 8,
    kategorie: "MANV 5+", 
    farbe: "info.main",
    status: "Bearbeitung",
...
// Create context for expanded menus
...
const userRoles = [
  {
    ...
    color: 'error.main',
    permissions: ['all'],
    description: 'Uneingeschränkter Zugriff auf alle Systembereiche'
  },
  {
    id: 'admin',
    name: 'Administrator',
    color: 'error.light',
    permissions: ['administration', 'benutzerverwaltung', 'auswertung'],
    description: 'Verwaltung von Benutzern und Systemeinstellungen'
  },
  {
    id: 'leitstelle',
    name: 'Leitstelle',
    color: 'info.main',
    permissions: ['erfassung', 'auswertung', 'einsatz'],
    description: 'Koordination von Einsätzen und Ressourcen'
  },
  {
    id: 'erfasser',
    name: 'Erfasser',
    color: 'primary.main',
    permissions: ['erfassung'],
    description: 'Datenerfassung und -management'
  },
  {
    id: 'auswerter',
    ...
    permissions: ['auswertung', 'jahresstatistik'],
    description: 'Analyse und Auswertung von Daten'
  },
  {
    id: 'sirene',
    name: 'Sirene',
    color: 'warning.main',
    permissions: ['erfassung_sirene'],
    description: 'Verwaltung von Sirenenanlagen'
  }
];
...
function MainLayout() {
  ...
  // State für menuItems mit expandierbaren Untermenüs
  const [localMenuItems, setLocalMenuItems] = useState(menuItems.map(item => ({ ...item, isOpen: false })));
  ...
  // Dark Mode aus dem Context holen
  const { darkMode, toggleDarkMode } = useThemeContext();
  
  // Favorites Context - call hook at the top level
  const favorites = useFavoritesContext();
  
  // Benutzerinformationen als State mit mehreren Rollen
  const [currentUser, setCurrentUser] = useState({
    name: "yanik02",
    roles: ['erfasser', 'auswerter', 'superadmin', 'admin', 'leitstelle', 'sirene'], // Benutzer hat alle Rollen
    location: "BezReg Düsseldorf",
  });
  
  // State für Rendering von Admin und Benutzerverwaltung Untermenüs als Cards - auf false gesetzt
  const [showAdminCards, setShowAdminCards] = useState(false);
  const [showBenutzerCards, setShowBenutzerCards] = useState(false);
  
  // Funktion zum Prüfen, ob der Benutzer eine bestimmte Rolle hat
  const hasRole = (roleId) => {
    return currentUser.roles.includes(roleId);
  };
  
  // Funktion zum Abrufen aller Rollen-Objekte des Benutzers
  const getUserRoleObjects = () => {
    return currentUser.roles.map(roleId => 
      userRoles.find(role => role.id === roleId)
    ).filter(Boolean);
  };
  
  // Funktion zum Abrufen aller Berechtigungen aus allen Rollen
  const getAllPermissions = () => {
    const userRoleObjects = getUserRoleObjects();
  ...
  const drawer = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Main Menu - Menüpunkte nur anzeigen, wenn sie nicht in Favoriten sind */}
        ...
        {menuItems.map((item) => {
          return (
            <React.Fragment key={item.title}>
                ...
                <ListItemButton 
                  onClick={() => {
                    if (item.title === 'MediRIG') {
                      ...
                      // Administration mit CustomCards anzeigen
                      navigate(item.path);
                      // Nur das Expand-State umschalten, aber showAdminCards auf true lassen
                      if (item.subMenu) {
                    ...
                    } else if (item.title === 'Benutzerverwaltung') {
                      // Benutzerverwaltung mit CustomCards anzeigen
                      navigate(item.path);
                      // Nur das Expand-State umschalten, aber showBenutzerCards auf true lassen
                      if (item.subMenu) {
                        setExpandedMenus(prev => ({
                          ...prev,
                          [item.title]: !prev[item.title]
                      ...
                      navigate(item.path);
                      // Toggle expanded menu state when clicking on a menu item with subMenu
                      if (item.subMenu) {
                        setExpandedMenus(prev => ({
                          ...prev,
                          [item.title]: !prev[item.title]
                  ...
                  sx={{ 
                    borderLeft: `4px solid ${isActive(item.path) ? 
              ...
              {/* Für Administration und Benutzerverwaltung: CustomCards statt einfacher Menüpunkte verwenden */}
              {item.title === 'Administration' && item.subMenu && (
                <Collapse in={expandedMenus[item.title] || false} timeout="auto" unmountOnExit>
                  {showAdminCards ? (
                    <Box sx={{ pl: 2, pr: 2, pb: 2 }}>
                      <Grid container spacing={2} sx={{ mt: 1 }}>
                        {item.subMenu.map((subItem) => (
                          <Grid item xs={12} key={subItem.title}>
                            <CustomCard
                              title={subItem.title}
                              description="Verwaltungsbereich"
                              icon={getIconForMenuItem(subItem.title)}
                              path={subItem.path}
                      ...
                      {item.subMenu.map((subItem) => {
                        return (
                          <React.Fragment key={subItem.title}>
                            <ListItem disablePadding>
                              ...
                              >
                                <ListItemIcon sx={{ 
                                  ...
                                  {subItem.icon || getIconForMenuItem(subItem.title)}
                                </ListItemIcon>
                                <ListItemText 
                                  primary={
                                    <Typography 
                                ...
                                {subItem.subMenu && (
                                  <IconButton 
                                    ...
                                    onClick={(e) => {
                                      ...
                                      setExpandedMenus(prev => ({
                                        ...prev,
                                        [subItem.title]: !prev[subItem.title]
                                      }));
                                    }}
                                    sx={{ mr: 1, color: 'text.secondary' }}
                                  >
                                    {expandedMenus[subItem.title] ? <ExpandLessIcon fontSize="small" /> : <ExpandMoreIcon fontSize="small" />}
                                  </IconButton>
                                )}
                              </ListItemButton>
                            </ListItem>
                            ...
                            {subItem.subMenu && (
                              <Collapse in={expandedMenus[subItem.title] || false} timeout="auto" unmountOnExit>
                                <List component="div" disablePadding>
                                  {subItem.subMenu.map((subSubItem) => {
                                    return (
                                      <ListItem key={subSubItem.title} disablePadding>
                                        <ListItemButton 
                                          onClick={() => {
                                          ...
                                          sx={{ 
                                            ...
                                            py: 0.75,
                                            ml: 8,
                                            borderLeft: `1px solid ${isActive(subSubItem.path) ? 
                                              theme.palette[item.color].main : 
                                              'transparent'}`,
                                            '&.Mui-selected': {
                                              backgroundColor: 'rgba(0, 0, 0, 0.04)'
                                            ...
                                            '&:hover': {
                                              backgroundColor: 'rgba(0, 0, 0, 0.04)',
                                              borderLeft: `1px solid ${theme.palette[item.color].main}`
                                            }
                                          }}
                                        >
                                          <ListItemText 
                                            ...
                                            primary={
                                              <Typography 
                                                variant="body2" 
                                                sx={{ 
                                                  fontWeight: isActive(subSubItem.path) ? 'bold' : 'regular',
                                                  color: isActive(subSubItem.path) ? 'text.primary' : 'text.secondary',
                                                  fontSize: '0.85rem'
                                                }}
                                              >
                                                {subSubItem.title}
              ...
              {/* Für Benutzerverwaltung: CustomCards statt einfacher Menüpunkte verwenden */}
              {item.title === 'Benutzerverwaltung' && item.subMenu && (
                <Collapse in={expandedMenus[item.title] || false} timeout="auto" unmountOnExit>
                  {showBenutzerCards ? (
                    <Box sx={{ pl: 2, pr: 2, pb: 2 }}>
                      <Grid container spacing={2} sx={{ mt: 1 }}>
                        {item.subMenu.map((subItem) => (
                          <Grid item xs={12} key={subItem.title}>
                            <CustomCard
                              title={subItem.title}
                              description="Benutzerverwaltungsbereich"
                              icon={getIconForMenuItem(subItem.title)}
                              path={subItem.path}
                              accentColor={item.color}
                              onClick={() => {
              ...
              {/* Jahresstatistik und alle anderen Menüs bleiben unverändert */}
              {item.title !== 'Administration' && item.title !== 'Benutzerverwaltung' && item.subMenu && (
                <Collapse in={expandedMenus[item.title] || false} timeout="auto" unmountOnExit>
                  <List component="div" disablePadding>
                    {item.subMenu.map((subItem) => {
                      return (
                          ...
                          <ListItem disablePadding>
                            ...
                            >
                              <ListItemIcon sx={{ 
                                ...
                                {subItem.icon || <KeyboardArrowRightIcon fontSize="small" />}
                              </ListItemIcon>
                              <ListItemText 
                                primary={
                                  <Typography 
      ...
      {/* Favoriten-Bereich am unteren Ende der Sidebar */}
      <Box sx={{ mt: 'auto', mb: 2 }}>
        <Box sx={{ 
          display: 'flex', 
          alignItems: 'center', 
        ...
        {/* Favorites as interactive chips with hover effects */}
        <Box sx={{ 
          px: 2, 
          py: 0.5, 
          display: 'flex', 
          ...
          {/* Favoriten als kompakte, interaktive Chips */}
          {favorites && favorites.favorites && favorites.favorites.map((favorite, index) => (
            <Chip
              key={favorite.title + favorite.path}
              label={favorite.title}
              icon={getIconForMenuItem(favorite.iconType || favorite.title)}
              onClick={() => {
              ...
              onDelete={() => favorites.toggleFavorite(favorite)}
              deleteIcon={
                <Box 
                  sx={{ 
                    width: 16, 
              ...
              sx={{ 
                ...
                border: '1px solid',
                borderColor: isActive(favorite.path) 
                  ? favorite.accentColor 
                  : `${favorite.accentColor}30`,
                backgroundColor: isActive(favorite.path) 
                  ? `${favorite.accentColor}20` 
                  : theme.palette.mode === 'dark' 
                    ? 'rgba(255,255,255,0.05)' 
                    : 'rgba(0,0,0,0.02)',
                color: isActive(favorite.path) 
                  ? favorite.accentColor 
                  : theme.palette.text.secondary,
                fontWeight: isActive(favorite.path) ? 'medium' : 'normal',
                '& .MuiChip-icon': {
                  color: isActive(favorite.path) 
                    ? favorite.accentColor 
                    : `${favorite.accentColor}90`,
                  fontSize: '16px',
                  marginLeft: 0.5,
                  marginRight: -0.5,
                ...
                '&:hover': {
                  ...
                  boxShadow: `0 4px 8px ${favorite.accentColor}30`,
                  backgroundColor: `${favorite.accentColor}15`,
                  borderColor: `${favorite.accentColor}80`,
                  '& .MuiChip-icon': {
                ...
                '&:active': {
                  ...
                  boxShadow: `0 1px 3px ${favorite.accentColor}20`,
                  backgroundColor: `${favorite.accentColor}30`
                },
                animation: isActive(favorite.path) ? 
                  'pulse-glow 2s infinite alternate' : 'none',
                '@keyframes pulse-glow': {
                  '0%': { boxShadow: '0 0 2px rgba(0,0,0,0)' },
                  '100%': { boxShadow: `0 0 10px ${favorite.accentColor}50` }
                }
              }}
            />
          ))}
          
          {/* Falls keine Favoriten vorhanden sind */}
          {(!favorites || !favorites.favorites || favorites.favorites.length === 0) && (
            <Box sx={{ 
              width: '100%', 
              textAlign: 'center', 
              ...
              backgroundImage: theme.palette.mode === 'dark' 
                ? `linear-gradient(135deg, rgba(50,50,50,0.2) 25%, rgba(30,30,30,0.3) 25%, rgba(30,30,30,0.3) 50%, rgba(50,50,50,0.2) 50%, rgba(50,50,50,0.2) 75%, rgba(30,30,30,0.3) 75%, rgba(30,30,30,0.3) 100%)` 
                : `linear-gradient(135deg, ${theme.palette.background.paper} 25%, ${theme.palette.background.default} 25%, ${theme.palette.background.default} 50%, ${theme.palette.background.paper} 50%, ${theme.palette.background.paper} 75%, ${theme.palette.background.default} 75%, ${theme.palette.background.default} 100%)`,
              backgroundSize: '20px 20px',
              border: '1px dashed',
              borderColor: 'divider'
            }}>
              <Tooltip title="Favoriten hinzufügen">
                <BookmarksIcon 
                  sx={{ 
                    ...
                    '&:hover': {
                      transform: 'scale(1.1)',
                      color: 'primary.light',
                      filter: 'drop-shadow(0 2px 4px rgba(0, 90, 158, 0.3))'
                    }
                  }}
                />
              </Tooltip>
              ...
              >
                Klicken Sie den Stern bei Menüeinträgen an
              </Typography>
            </Box>
          )}
        </Box>
  ...
  return (
    <ExpandedMenuContext.Provider value={{ expandedMenus, setExpandedMenus }}>
      <Box sx={{ display: 'flex', height: '100vh' }}>
        ...
        >
          <Toolbar sx={{ 
            ...
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Tooltip title="Benachrichtigungen">
                <IconButton 
                  ...
                  sx={{ 
                    ...
                    position: 'relative',
                    '&:hover': {
                      backgroundColor: 'rgba(0,90,158,0.07)'
                    } 
                  }}
                >
              ...
              <Tooltip title={darkMode ? "Light Mode" : "Dark Mode"}>
                <IconButton 
                  ...
                  sx={{ 
                    mx: 1,
                    '&:hover': {
                      backgroundColor: darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,90,158,0.07)'
                    } 
                  }}
                >
              ...
              >
                {/* Farbiger Rand basierend auf Rollen */}
                <Box 
                  sx={{ 
                    position: 'absolute',
                    top: 0,
                    ...
                    background: 'linear-gradient(90deg, ' + 
                      getUserRoleObjects().map(role => theme.palette[role.color.split('.')[0]][role.color.split('.')[1] || 'main']).join(', ') + 
                    ')'
                  }} 
                />
                ...
                <Box sx={{ ml: 1.5, mr: 0.5, display: { xs: 'none', sm: 'block' } }}>
                  <Typography variant="body2" sx={{ fontWeight: 'bold', lineHeight: 1.2 }}>
                    {currentUser.name}
                  </Typography>
                  <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                    {currentUser.roles.length} Rollen
                  </Typography>
                </Box>
                <KeyboardArrowDownIcon fontSize="small" sx={{ color: 'text.secondary', opacity: 0.7 }} />
              </Box>
            </Box>
          </Toolbar>
        ...
        {/* User Profile Menu - Überarbeitet für mehrere Rollen */}
        <Menu
          anchorEl={anchorEl}
        ...
        >
          ...
          <Box sx={{ p: 2, borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <Box>
                <Typography variant="subtitle1" fontWeight="bold">{currentUser.name}</Typography>
                <Typography variant="caption" color="text.secondary">{currentUser.location}</Typography>
              </Box>
            </Box>
          </Box>
          ...
          {/* Alle Rollen anzeigen */}
          <Box sx={{ p: 2, backgroundColor: 'background.default', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>Aktive Rollen</Typography>
            <Box sx={{ 
              display: 'flex', 
              flexWrap: 'wrap', 
              gap: 1,
          ...
          <Box sx={{ p: 2 }}>
            <Typography variant="body2" color="text.secondary" gutterBottom>Verfügbare Berechtigungen</Typography>
            <Box sx={{ 
              mt: 1,
              p: 1.5,
              borderRadius: 1,
              ...
              {getAllPermissions().includes('all') ? (
                <Typography variant="body2">
                  <strong>Vollzugriff</strong> auf alle Systembereiche
                </Typography>
              ) : (
        ...
        >
          <Container maxWidth="xl" sx={{ py: 3,paddingTop:'100px' }}>
            <Outlet />
          </Container>
        </Box>
      </Box>
...
const SidebarMenuItem = ({ item, isSelected, onMenuItemClick, isFavorite, onToggleFavorite }) => {
  const location = useLocation();
  const theme = useTheme();
