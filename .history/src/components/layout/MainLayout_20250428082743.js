...
// Liste der möglichen Benutzerrollen mit zugehörigen Farben und Icons
const userRoles = {
...
function MainLayout() {
  ...
  // Extract path segments for breadcrumb navigation
                  ...
                  {item.subMenu.map((subItem) => (
                    <ListItem key={subItem.title} disablePadding>
                      ...
                      >
                        ...
                        />
                        {/* Add favorite icon for submenu items */}
                        {favorites.findIndex(fav => 
                          fav.title === subItem.title && 
                          fav.path === subItem.path
                        ) === -1 && (
                          <IconButton 
                            size="small" 
                            onClick={(e) => {
                              e.stopPropagation();
                              // Pass a modified item that includes both the subItem properties and necessary parent properties
                              toggleFavorite({
                                ...subItem,
                                color: item.color, // Inherit color from parent
                                isSubmenu: true,
      ...
      <Box sx={{ display: 'flex', height: '100vh' }}>
        ...
        >
          <Toolbar sx={{ 
            ...
            {/* Left side: Menu icon and logo/title with animation */}
            <Box sx={{ 
              display: 'flex', 
              alignItems: 'center',
              position: 'relative'
              ...
              >
                <Box component="span" sx={{ 
                  background: 'linear-gradient(45deg, #004c8c 30%, #0277bd 70%, #42a5f5 90%)',
                  backgroundSize: '200% 100%',
              ...
              <Box 
                sx={{ 
                  ...
                  fontWeight: 'bold',
                  letterSpacing: '0.5px',
                  boxShadow: '0 2px 8px rgba(0, 90, 158, 0.25)',
                  position: 'relative',
                  overflow: 'hidden',
                  '&::before': {
                    content: '""',
            ...
            {/* Center section: Dynamic path indicators */}
            <Box 
              sx={{ 
                ...
                boxShadow: location.pathname !== '/' ? '0 2px 12px rgba(0,0,0,0.08)' : 'none',
                ...
                <Box sx={{ 
                  ...
                  {pathSegments.length > 0 && (
                    <>
                      ...
                      >
                        {menuItems.find(item => item.path === `/${pathSegments[0]}`)?.icon && (
                          <Box 
                            component="span" 
                            sx={{ 
                              mr: 1, 
                              display: 'flex', 
                              alignItems: 'center',
                              color: menuItems.find(item => item.path === `/${pathSegments[0]}`)?.color 
                                ? theme.palette[menuItems.find(item => item.path === `/${pathSegments[0]}`)?.color].main
                                : 'inherit',
                        ...
                        <Link 
                          ...
                          sx={{ 
                            ...
                            '&:hover': { 
                              color: menuItems.find(item => item.path === `/${pathSegments[0]}`)?.color 
                                ? theme.palette[menuItems.find(item => item.path === `/${pathSegments[0]}`)?.color].dark
                                : 'primary.dark'
                            }
                          }}
                        >
                        ...
                        {pathSegments.length > 1 && (
                          <>
                            <NavigateNextIcon fontSize="small" sx={{ mx: 0.5, opacity: 0.7 }} />
                            <Link 
                              component={RouterLink} 
            ...
            {/* Right side: Tools and quick actions with redesigned components */}
            <Box sx={{ 
              display: 'flex', 
              alignItems: 'center', 
              gap: 0.5
            }}>
              {/* Search Button with new animation and visual style */}
              <Tooltip 
                title="Suchen" 
                arrow
                placement="bottom"
                ...
                <IconButton 
                  ...
                  sx={{ 
                    ...
                    '&::before': {
                      ...
                      right: 0,
                      bottom: 0,
                      background: 'radial-gradient(circle at center, rgba(25, 118, 210, 0.2) 0%, rgba(25, 118, 210, 0) 70%)',
                      opacity: 0,
              ...
              {/* Dark Mode Toggle with improved visuals */}
              <Tooltip 
                  ...
                  <Box sx={{ 
                    ...
                    {darkMode ? (
                      <Brightness7Icon 
                        sx={{ 
                          position: 'absolute',
                          animation: 'fadeRotate 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) forwards',
                          color: 'warning.main',
              ...
              {/* User Profile with enhanced visual aesthetics */}
              <Tooltip 
                ...
                >
                  <Avatar 
                    sx={{ 
                      width: 32, 
                      height: 32,
                      background: userRoles[currentUser.role]?.color 
                        ? `linear-gradient(135deg, ${theme.palette[userRoles[currentUser.role].color].dark} 30%, ${theme.palette[userRoles[currentUser.role].color].main} 90%)`
                      ...
                      {userRoles[currentUser.role] && (
                        <Box 
                          ...
                          sx={{ 
                            ...
                            alignItems: 'center',
                            mr: 0.5,
                            color: theme.palette[userRoles[currentUser.role].color].main,
                            fontSize: '1rem'
                          }}
                        >
                          {userRoles[currentUser.role].icon}
                        </Box>
                      )}
                      <Typography 
                        variant="caption" 
                        sx={{ 
                          color: userRoles[currentUser.role]?.color 
                            ? theme.palette[userRoles[currentUser.role].color].main 
                            : 'text.secondary',
                          fontWeight: 'medium',
                          lineHeight: 1.2
                        }}
        ...
        >
          <Container maxWidth="xl" sx={{ py: 3, paddingTop: 13 }}>
            <Outlet />
          </Container>
        </Box>
      </Box>
