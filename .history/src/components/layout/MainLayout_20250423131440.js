  ...
  const drawer = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ mt: 'auto', mb: 2 }}>
        ...
        <List sx={{ py: 0 }}>
          {/* Favoriten aus dem Context anzeigen */}
          {useFavoritesContext && useFavoritesContext().favorites && useFavoritesContext().favorites.map((favorite) => (
            <ListItem key={favorite.title + favorite.path} disablePadding>
              <ListItemButton 
                onClick={() => {
                ...
                selected={isActive(favorite.path)}
                sx={{ 
                  py: 1,
                  borderLeft: `3px solid ${isActive(favorite.path) ? 
                    favorite.accentColor : 
                    'transparent'}`,
                  '&.Mui-selected': {
                    backgroundColor: `${favorite.accentColor}20`
                  },
                  '&:hover': {
                    backgroundColor: `${favorite.accentColor}10`
                  }
                }}
              >
                <ListItemIcon sx={{ 
                  ...
                  {getIconForMenuItem(favorite.iconType || favorite.title)}
                </ListItemIcon>
                <ListItemText 
                  primary={
                    <Typography 
                      variant="body2" 
                      sx={{ 
                        fontWeight: isActive(favorite.path) ? 'bold' : 'regular',
                        color: isActive(favorite.path) ? 'text.primary' : 'text.secondary'
                      }}
                    >
                      {favorite.title}
                    </Typography>
                ...
                <Tooltip title="Aus Favoriten entfernen">
                  <IconButton 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      useFavoritesContext().toggleFavorite(favorite);
                    }}
          ...
          {(!useFavoritesContext || !useFavoritesContext().favorites || useFavoritesContext().favorites.length === 0) && (
            <ListItem sx={{ py: 2 }}>
              <Box sx={{ 
                width: '100%', 
                textAlign: 'center', 
                ...
                <BookmarksIcon sx={{ color: 'text.disabled', fontSize: 32, mb: 1 }} />
                <Typography variant="body2" color="text.secondary">
                  Keine Favoriten vorhanden
                </Typography>
                <Typography variant="caption" color="text.disabled" sx={{ display: 'block', mt: 0.5 }}>
                  Klicken Sie auf den Stern bei Menüeinträgen, um Favoriten hinzuzufügen
                </Typography>
              </Box>
            </ListItem>
          )}
