...
const MenuItem = ({ item, isSelected, onMenuItemClick, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <>
      ...
      >
        <ListItemIcon sx={{ color: isSelected ? `${item.color}.dark` : 'inherit' }}>
          {item.icon}
        </ListItemIcon>
        <ListItemText 
          primary={item.title} 
        ...
        />
        {hasSubMenu && (isExpanded ? <ExpandLessIcon /> : <ExpandMoreIcon />)}
        <IconButton 
          onClick={(e) => {
            e.stopPropagation();
          ...
          {item.subMenu.map((subItem) => (
            <ListItemButton
              ...
              sx={{
                ...
                py: 0.5,
                borderRadius: 2,
                backgroundColor: location.pathname === subItem.path ? `${item.color || 'primary'}.lighter` : 'transparent',
                color: location.pathname === subItem.path ? `${item.color || 'primary'}.dark` : 'inherit',
                '&:hover': {
                  backgroundColor: location.pathname === subItem.path ? `${item.color || 'primary'}.lighter` : 'grey.100'
                }
              }}
            >
              <ListItemText 
